...
def app_instance():
    ...
    return E2bApp(integration=mock_integration)

def test_universal_mcp_e2b_app_initialization(app_instance):
    """
    Test that the E2bApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "e2b", "E2bApp instance has unexpected name."


def test_universal_mcp_e2b_tool_docstrings_format(app_instance):
    """
    Test that each tool method in E2bApp has a well-formatted docstring,
...
def test_universal_mcp_e2b_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in E2bApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
